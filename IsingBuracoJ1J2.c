...
int update(double beta) {
  ...
  for(j=0; j<N; j++) {
    ...
    E1=-J1*lat[i]*(lat[r]+lat[l]+lat[u]+lat[d])-J2*lat[i]*(lat[ul]+lat[ur]+lat[dl]+lat[dr])-H*lat[i]; //energy of the spin i in your state
    E2= J1*lat[i]*(lat[r]+lat[l]+lat[u]+lat[d])+J2*lat[i]*(lat[ul]+lat[ur]+lat[dl]+lat[dr])+H*lat[i]; //energy of the spin i in the other state
